...
The following actions will be necessary to successfully configure your development environment:
  ...
  * venv created within root project directory
  
From ```root``` directory:
  - run ```pip install -e .```
  - run ```pip install -r requirements.txt```
From ```autogen```
  - run ```npm run openapi-server-run-all```
From ```autogen/server/release```:
  - run ```pip install -e .```
...
in the domains module. Each domain follows the general structure of ```controllers/services/tests```. After ensuring
all unit tests are run and have passed, run:
